...
class FlxFrame implements IFlxDestroyable
{
	...
	{
		return getNameOrderLegacy(frame1.name, prefixLength, suffixLength) - getNameOrderLegacy(frame2.name, prefixLength, suffixLength);
	}
	...
	{
		inline function logWarning(msg)
		{
			if (warn)
				FlxG.log.warn(msg);
		}
		...
		final start = prefix != null ? prefix.length : 0;
		final section = name.substring(start, end);
		final order:Null<Int> = Std.parseInt(section);
